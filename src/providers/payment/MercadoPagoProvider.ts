...
export class MercadoPagoProvider implements PaymentProvider {
  ...
  async iniciarPagamento(input: IniciarPagamentoInput): Promise<ResultadoPagamento> {
    ...
    if (!qrCode || !qrCodeBase64 || !resultado.id) {
      throw new Error("Resposta do Mercado Pago veio sem dados do QR Code");
    }
  ...
  async validarWebhook(payload: PayloadWebhook, assinatura: string): Promise<ResultadoWebhook> {
    ...
    if (!assinaturaValida) {
      throw new Error("Assinatura do webhook do Mercado Pago é inválida");
    }

    const statusAtual = await this.consultarStatus(payload.dataId);

    return {
      referenciaExterna: payload.dataId,
